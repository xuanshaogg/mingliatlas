...
export function buildLlmsText(): string {
  ...
  return [
    ...
    "## Topic References",
    "",
    "The following pages explain the site's main topics. Consult their source notes for the scope of each explanation:",
    ...
    `- "How do Bazi Luck Pillars work": ${SITE.url}/bazi/luck-pillars — 10-year stem-branch cycles compared with the natal chart; the browser calculator shows Da Yun direction, starting date, and cycles when male or female is selected for the traditional calculation rule. Without that selection, the natal chart remains available without Da Yun.`,
    ...
    "## Discovery Files",
    "",
    `- [Full LLM index](${SITE.url}/llms-full.txt): Indexable pages grouped by section with descriptions.`,
    ...
    "Each page includes trigram composition, editorial summaries of Judgment and Image themes, modern reading suggestions and source notes. These summaries are not verbatim translations.",
    "",
    "Selected indexable hexagram guides:",
    ...indexableSitePages
      .filter((page) => /^\/i-ching\/hexagram-\d+$/.test(page.href))
      .map(markdownLink),
    "",
...
export function buildLlmsFullText(): string {
  ...
  const lines = [
    ...
    "",
    `Generated from the site indexing policy for LLM and agent discovery. Last updated: ${latestModified()}.`,
    "",
  ];
