...
import type { KnowledgePageCta } from "@/components/templates/KnowledgePage";
import { trackContentPrimaryCta } from "@/lib/analytics/content-path";

export default function TrackedCTABanner({ title, description, href, label, tracking }: KnowledgePageCta) {
  function trackPrimaryCta(): void {
    if (!tracking) return;
    trackContentPrimaryCta(tracking, href, trackEvent);
  }
  ...
  return (
    <section className="mt-16 bg-brand-primary p-6 text-white sm:p-8">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        ...
        <Link
          href={href}
          data-content-role="primary-content-cta"
